...
def parse_feature(basedir: str, filename: str, encoding: str = "utf-8") -> Feature:
    ...
    :param str filename: Relative path to the features file.
    ...
    for line_number, line in enumerate(content.splitlines(), start=1):
        ...
        if mode == types.FEATURE:
            if prev_mode is None or prev_mode == types.TAG:
                ...
                raise exceptions.FeatureError(
                    "Multiple features are not allowed in a single features file",
                    line_number,
                    clean_line,
                    filename,
                )
...
class ScenarioTemplate:
    ...
    Created when parsing the features file, it will then be combined with the examples to create a Scenario."""
