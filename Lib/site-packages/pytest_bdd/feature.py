...
def get_feature(base_path: str, filename: str, encoding: str = "utf-8") -> Feature:
    """Get a features by the filename.

    :param str base_path: Base features directory.
    :param str filename: Filename of the features file.
    :param str encoding: Feature file encoding.

    :return: `Feature` instance from the parsed features cache.
    ...
    for path in paths:
        if path not in seen_names:
            ...
            if os.path.isdir(path):
                features.extend(get_features(glob2.iglob(os.path.join(path, "**", "*.features")), **kwargs))
            else:
