...
def add_options(parser: Parser) -> None:
    ...
    group._addoption(
        ...
        default=False,
        help="Generate missing bdd test code for given features files and exit.",
    )
...
def print_missing_code(scenarios: list[ScenarioTemplate], steps: list[Step]) -> None:
    ...
    for scenario in scenarios:
        tw.line()
        tw.line(
            'Scenario "{scenario.name}" is not bound to any test in the features "{scenario.features.name}"'
            " in the file {scenario.features.filename}:{scenario.line_number}".format(scenario=scenario),
            red=True,
        )
    ...
    for step in steps:
        ...
        if step.scenario is not None:
            tw.line(
                """Step {step} is not defined in the scenario "{step.scenario.name}" in the features"""
                """ "{step.scenario.features.name}" in the file"""
                """ {step.scenario.features.filename}:{step.line_number}""".format(step=step),
                red=True,
            )
        elif step.background is not None:
            tw.line(
                """Step {step} is not defined in the background of the features"""
                """ "{step.background.features.name}" in the file"""
                """ {step.background.features.filename}:{step.line_number}""".format(step=step),
                red=True,
            )
...
def parse_feature_files(paths: list[str], **kwargs: Any) -> tuple[list[Feature], list[ScenarioTemplate], list[Step]]:
    """Parse features files of given paths.
...
def _show_missing_code_main(config: Config, session: Session) -> None:
    ...
    if config.option.features is None:
        tw.line("The --features parameter is required.", red=True)
